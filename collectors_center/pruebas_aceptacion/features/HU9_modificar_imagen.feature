Característica: Agregar articulo
    Como joven coleccionista deseo poder cambiar las imagenes
    de mis articulos para mostrar fotos mas atractivas o 
    actualizadas en mi coleccion


    Escenario: Agregar articulo descripcion
            Dado que abro la aplicación
            Y espero un rato
            Y doy click en el boton Ingresar
            Y espero un rato
            Y ingreso mi correo "test123@gmail.com"
            Y luego ingreso mi contraseña "Test123!"
            Y presiono el botón Acceder
            Y presiono el boton hamburguesa
            Y presiono Categorias
            Y selecciono la categoria "pitufos"
            Y le doy click en un articulo
            Y subo la imagen "imagen.jpg"
            Y presiono Guardar
            Entonces puedo ver el mensaje "Imagen modifcada correctamente"
